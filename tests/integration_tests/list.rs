...
"#,
    ...
    repo.setup_remote("main");
    // setup_remote() is a no-op when origin already exists from the standard
    // fixture, so the prior commit hasn't been pushed yet — push so local
    // main and origin/main agree before any wt merge runs.
